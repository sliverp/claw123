...
export async function PATCH(
  ...
  try {
    ...
    const review = get<{ id: number; claw_id: number; rating: number }>(
    ...
    execute('UPDATE reviews SET approved = ? WHERE id = ?', [approvedValue, reviewId]);

    // 重新计算该 claw 的统计数据（仅已审核通过的评价）
    const stats = get<{ avg_r: number; cnt: number }>(
    ...
    execute(
      ...
      [review.claw_id, stats?.avg_r || 0, stats?.cnt || 0]
    );
