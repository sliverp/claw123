...
export async function GET(
  ...
  try {
    ...
    const reviews = await query(
      'SELECT id, nickname, content, created_at FROM benchmarks_reviews WHERE item_id = ? AND approved = 1 ORDER BY created_at DESC',
      [benchmark.id]
    );
    ...
    if (fp) {
      const existing = await get(
        'SELECT id FROM benchmarks_reviews WHERE item_id = ? AND (ip = ? OR fingerprint = ?)',
        [benchmark.id, ip, fp]
      );
      hasReviewed = !!existing;
    } else {
      const existing = await get(
        'SELECT id FROM benchmarks_reviews WHERE item_id = ? AND ip = ?',
        [benchmark.id, ip]
      );
      hasReviewed = !!existing;
    }
    ...
    const existingRating = await get<{ id: number; rating: number }>(
      'SELECT id, rating FROM benchmarks_ratings WHERE item_id = ? AND ip = ?',
      [benchmark.id, ip]
    );
    if (existingRating) {
      hasRated = true;
    ...
    console.error('Failed to fetch benchmark reviews:', error);
...
export async function POST(
  ...
  try {
    ...
    await execute(
      'INSERT INTO benchmarks_reviews (item_id, nickname, content, ip, fingerprint, approved) VALUES (?, ?, ?, ?, ?, 0)',
      [benchmark.id, safeNickname, content.trim(), ip, fp]
    );
    ...
    console.error('Failed to create benchmark review:', error);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}
