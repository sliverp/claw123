...
export async function POST(
  ...
  try {
    ...
    const existing = await get<{ id: number }>(
      'SELECT id FROM token_coding_plans_ratings WHERE item_id = ? AND ip = ?',
      [item.id, ip]
    );
    if (existing) {
    ...
    await execute(
      'INSERT INTO token_coding_plans_ratings (item_id, ip, rating) VALUES (?, ?, ?)',
      [item.id, ip, Math.floor(Number(rating))]
    );

    const stats = await query<{ avg_r: number; cnt: number }>(
      'SELECT AVG(rating) as avg_r, COUNT(*) as cnt FROM token_coding_plans_ratings WHERE item_id = ?',
      [item.id]
    );

    if (stats.length > 0) {
      const upsertSQL = isMySQL
        ? `INSERT INTO token_coding_plans_stats (item_id, avg_rating, review_count) VALUES (?, ?, ?)
      ...
      await execute(upsertSQL, [item.id, stats[0].avg_r, stats[0].cnt]);
    }

    return NextResponse.json({ success: true }, { status: 201 });
  } catch (error: unknown) {
    console.error('Failed to rate token coding plan:', error);
