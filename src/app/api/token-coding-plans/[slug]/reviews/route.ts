...
export async function GET(
  ...
  try {
    ...
    const reviews = await query(
      'SELECT id, nickname, content, created_at FROM token_coding_plans_reviews WHERE item_id = ? AND approved = 1 ORDER BY created_at DESC',
      [item.id]
    );
    ...
    if (fp) {
      const existing = await get(
        'SELECT id FROM token_coding_plans_reviews WHERE item_id = ? AND (ip = ? OR fingerprint = ?)',
        [item.id, ip, fp]
      );
      hasReviewed = !!existing;
    } else {
      const existing = await get(
        'SELECT id FROM token_coding_plans_reviews WHERE item_id = ? AND ip = ?',
        [item.id, ip]
      );
      hasReviewed = !!existing;
    }
    ...
    const existingRating = await get<{ id: number; rating: number }>(
      'SELECT id, rating FROM token_coding_plans_ratings WHERE item_id = ? AND ip = ?',
      [item.id, ip]
    );
    if (existingRating) {
      hasRated = true;
    ...
    console.error('Failed to fetch token coding plan reviews:', error);
...
export async function POST(
  ...
  try {
    ...
    await execute(
      'INSERT INTO token_coding_plans_reviews (item_id, nickname, content, ip, fingerprint, approved) VALUES (?, ?, ?, ?, ?, 0)',
      [item.id, safeNickname, content.trim(), ip, fp]
    );
    ...
    console.error('Failed to create token coding plan review:', error);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}
