...
export async function POST(
  ...
  try {
    ...
    const existing = await get<{ id: number }>(
      'SELECT id FROM frameworks_ratings WHERE item_id = ? AND ip = ?',
      [framework.id, ip]
    );
    if (existing) {
    ...
    await execute(
      'INSERT INTO frameworks_ratings (item_id, ip, rating) VALUES (?, ?, ?)',
      [framework.id, ip, Math.floor(Number(rating))]
    );

    const stats = await query<{ avg_r: number; cnt: number }>(
      'SELECT AVG(rating) as avg_r, COUNT(*) as cnt FROM frameworks_ratings WHERE item_id = ?',
      [framework.id]
    );

    if (stats.length > 0) {
      const upsertSQL = isMySQL
        ? `INSERT INTO frameworks_stats (item_id, avg_rating, review_count) VALUES (?, ?, ?)
      ...
      await execute(upsertSQL, [framework.id, stats[0].avg_r, stats[0].cnt]);
    }

    return NextResponse.json({ success: true }, { status: 201 });
  } catch (error: unknown) {
    console.error('Failed to rate framework:', error);
