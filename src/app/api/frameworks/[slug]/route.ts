...
export async function GET(
  ...
  try {
    ...
    const row = await get<Record<string, unknown>>(
      `SELECT f.*, COALESCE(st.avg_rating, 0) as avg_rating, COALESCE(st.review_count, 0) as review_count, COALESCE(st.visit_count, 0) as visit_count
       FROM frameworks f
       LEFT JOIN frameworks_stats st ON f.id = st.item_id
       WHERE f.slug = ?`,
      [params.slug]
    );
