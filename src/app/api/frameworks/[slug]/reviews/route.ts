...
export async function GET(
  ...
  try {
    ...
    const reviews = await query(
      'SELECT id, nickname, content, created_at FROM frameworks_reviews WHERE item_id = ? AND approved = 1 ORDER BY created_at DESC',
      [framework.id]
    );
    ...
    if (fp) {
      const existing = await get(
        'SELECT id FROM frameworks_reviews WHERE item_id = ? AND (ip = ? OR fingerprint = ?)',
        [framework.id, ip, fp]
      );
      hasReviewed = !!existing;
    } else {
      const existing = await get(
        'SELECT id FROM frameworks_reviews WHERE item_id = ? AND ip = ?',
        [framework.id, ip]
      );
      hasReviewed = !!existing;
    }
    ...
    const existingRating = await get<{ id: number; rating: number }>(
      'SELECT id, rating FROM frameworks_ratings WHERE item_id = ? AND ip = ?',
      [framework.id, ip]
    );
    if (existingRating) {
      hasRated = true;
    ...
    console.error('Failed to fetch framework reviews:', error);
...
export async function POST(
  ...
  try {
    ...
    await execute(
      'INSERT INTO frameworks_reviews (item_id, nickname, content, ip, fingerprint, approved) VALUES (?, ?, ?, ?, ?, 0)',
      [framework.id, safeNickname, content.trim(), ip, fp]
    );
    ...
    console.error('Failed to create framework review:', error);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}
