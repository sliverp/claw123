...
import { SectionKey, SECTIONS, ItemWithStats } from '@/lib/types';
...
export default function GenericSection({ sectionKey, sortMode, onSortChange }: Props) {
  ...
  const grouped = useMemo(() => {
    if (sortMode !== 'alpha') {
      return [{ letter: 'ALL', items: sortedItems }];
    }
    const map: Record<string, ItemWithStats[]> = {};
    for (const item of sortedItems) {
      const letter = getAlphaGroup(item.name);
  ...
  useEffect(() => {
    ...
    if (grouped.length > 0 && !activeLetter) {
        ...
        <main className="flex-1 min-w-0">
          {loading ? (
            ...
            <div className="text-center py-20">
              <div className="text-6xl mb-4">{sectionDef.icon}</div>
              <h3 className="text-lg font-semibold text-slate-600 mb-2">{sectionDef.label}</h3>
              <p className="text-slate-400 mb-6">{sectionDef.description}</p>
