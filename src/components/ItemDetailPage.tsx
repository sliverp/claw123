...
interface Props<T extends DetailItem> {
  slug: string;
  apiBase: string;
}

export default function ItemDetailPage<T extends DetailItem>({ slug, apiBase }: Props<T>) {
  ...
  const handleRated = () => {
    setHasRated(true);
    fetch(`${apiBase}/${slug}`).then((res) => {
      if (res.ok) res.json().then(setItem);
    });
  };
